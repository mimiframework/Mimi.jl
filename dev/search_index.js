...

{
    ...
    "category": "section",
    "text": "Finally, use run which runs a simulation, indicating the sim_def, the models is a model, marginal model, or list of models to be run by your sim_def simulation, and samplesize the number of samples to use.In it\'s simplest use, the run function generates and iterates over generated trial data, perturbing a chosen subset of Mimi\'s \"external parameters\", based on the defined distributions, and then runs the given Mimi model(s). The function retuns an instance of SimulationInstance, holding a copy of the original SimulationDef in addition to trials information (trials, current_trial, and current_data), the model listmodels, and results information in results. Optionally, trial values and/or model results are saved to CSV files. Optionally, trial values and/or model results are saved to CSV files.  Note that if there is concern about in-memory storage space for the results, use the results_in_memory flag set to false to incrementally clear the results from memory. View the internals documentation for critical and useful details on the full signature of this function:function Base.run(sim_def::SimulationDef{T}, models::Union{Vector{Model}, Model}, samplesize::Int;\n                 ntimesteps::Int=typemax(Int), \n                 trials_output_filename::Union{Nothing, AbstractString}=nothing, \n                 results_output_dir::Union{Nothing, AbstractString}=nothing, \n                 pre_trial_func::Union{Nothing, Function}=nothing, \n                 post_trial_func::Union{Nothing, Function}=nothing,\n                 scenario_func::Union{Nothing, Function}=nothing,\n                 scenario_placement::ScenarioLoopPlacement=OUTER,\n                 scenario_args=nothing,\n                 results_in_memory::Bool=true) where T <: AbstractSimulationDataHere, we first employ run to obtain results:\n# Run 100 trials and save results to the indicated directories, one CSV file per RV for the results\nsi = run(sd, m, 100; trials_output_filename = \"/tmp/trialdata.csv\", results_output_dir=\"/tmp/Mimi\")\n\n# Explore the results saved in-memory\nresults = getdataframe(si, :grosseconomy, :K) # model index chosen defaults to 1and then again using our user-defined post-trial function as the post_trial_func parameter:# Same thing but with a post-trial function\nsi = run(sd, m, 100; trials_output_filename = \"/tmp/trialdata.csv\", results_output_dir=\"/tmp/Mimi\", post_trial_func=print_result)\n\n# Explore the results saved in-memory\nresults = getdataframe(si, :grosseconomy, :K) # model index chosen defaults to 1"
},

{
    "location": "tutorials/tutorial_4/#Step-5.-Explore-and-Plot-Results-1",
    "page": "4 Sensitivity Analysis",
    "title": "Step 5. Explore and Plot Results",
    "category": "section",
    "text": "As described in the internals documentation here, Mimi provides both explore and Mimi.plot to explore the results of both a run Model and a run SimulationInstance. To view your results in an interactive application viewer, simply call:explore(si)If desired, you may also include a title for your application window. If more than one model was run in your Simulation, indicate which model you would like to explore with the model keyword argument, which defaults to 1. Finally, if your model leverages different scenarios, you must indicate the scenario_name.explore(si; title = \"MyWindow\", model = 1) # we do not indicate scen_name here since we have no scenariosTo view the results for one of the saved variables from the save command in @defsim, use the (unexported to avoid namespace collisions)Mimi.plot function.  This function has the same keyword arguments and requirements as explore, save for title, and three required arguments: the SimulationInstance, the component name (as a Symbol), and the variable name (as a Symbol).using VegaLite\nMimi.plot(si, :grosseconomy, :K)To save your figure, use the save function to save typical file formats such as PNG, SVG, PDF and EPS files. Note that while explore(sim_inst) returns interactive plots for several graphs, Mimi.plot(si, :foo, :bar) will return only static plots. p = Mimi.plot(si, :grosseconomy, :K)\nsave(\"MyFigure.png\", p)"
},
...

{
    ...
    "category": "section",
    "text": "A small set of unexported functions are available to modify an existing SimulationDefinition.  The functions include:delete_RV!\nadd_RV!\nreplace_RV!\ndelete_transform!\nadd_transform!\ndelete_save!\nadd_save!\nset_payload!\npayload"
},
