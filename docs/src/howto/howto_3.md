...
The macro next defines how to apply the values generated by each RV to model parameters based on a pseudo-assignment operator. The left hand side of these assignments can be either a `param`, which must refer to a shared external parameter, or `comp.param` which refers to an unshared external parameter specific to a component.
